...
public abstract class QueryBuilderBase<QB> : IDisposable where QB : QueryBuilderBase<QB>
{
    ...
    protected QueryBuilderBase(World world, Span<TypeExpression> streamTypes)
    {
        _world = world;
        ...
        if (!_mask.HasTypes.Contains(TypeExpression.Of<Identity>(Match.Plain)))
            _mask.Has(TypeExpression.Of<Identity>(Match.Plain));
    }
